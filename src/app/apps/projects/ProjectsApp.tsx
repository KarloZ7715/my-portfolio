...
export function ProjectsApp() {
  ...
  return (
    <div className="flex h-full text-[color:var(--macos-text-primary)]">
      <div className="flex-1 overflow-auto macos-scroll p-6">
        <div className="mb-4">
        ...
        <div className="grid grid-cols-2 gap-3" style={{ perspective: 800 }}>
          {PROJECTS.map((p) => (
            <ProjectCard key={p.slug} project={p} active={p.slug === selected.slug} onClick={() => pick(p.slug)} />
          ))}
        </div>
      </div>

      <aside className="w-80 shrink-0 h-full overflow-auto macos-scroll p-5 bg-[color:var(--macos-sidebar-bg)] border-l border-[color:var(--macos-hairline)]">
